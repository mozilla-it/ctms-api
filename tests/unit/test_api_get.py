...
def test_get_ctms_for_maximal_contact(client, maximal_contact_single_response):
    """GET /ctms/{email_id} returns a contact with almost all fields set."""
    email_id = maximal_contact_single_response.email.email_id
    resp = client.get(f"/ctms/{email_id}")
    assert resp.status_code == 200
    assert resp.json() == jsonable_encoder(maximal_contact_single_response)

...
cat tests/integration/basket-db-init.sql | docker-compose exec -T mysql mysql -u root -h mysql basket

# Create token in CTMS (will only work with specific CTMS_SECRET, see .sql source)
cat tests/integration/ctms-db-init.sql | docker-compose exec -T postgres psql --user postgres -d postgres
